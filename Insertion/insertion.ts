import {
  CourseModel,
  PrerequisiteModel,
} from "../Collections/CoursePrerequisite";
  ...
  try {
    ...
    for (let item of data) {
      if (item.p_level === "null" && item.p_name === "null") {
        ...
        const cid = await CourseModel.findOne(
          { course_level: item.course_level, name: item.name },
          { _id: 1 }
        );
        const pid = await CourseModel.findOne(
          { course_level: item.p_level, name: item.p_name },
          { _id: 1 }
        );
        if (cid && pid) {
          ...
          if (!prerequisite) {
            ...
            await prerequisite.prerequisiteIds.push(pid._id);
          }
        }
      }
    }
